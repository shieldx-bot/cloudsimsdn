...
public abstract class NetworkOperatingSystem extends SimEntity {
    ...
    private final FaultInjectorService faultService = new FaultInjectorService();

    public FaultInjectorService getFaultService() {
        return faultService;
    }
	...
	public void processEvent(SimEvent ev) {
		...
		switch(tag){
			...
			case CloudSimTagsSDN.MONITOR_UPDATE_UTILIZATION:
				...
				this.updateFaultMonitor(CloudSim.clock());
...
//				NetworkOperatingSystem.getVmName(dst) + ", flow ="+flowId + " / eft="+eft);
	...
	private void updateFaultMonitor(double logTime) {
		if(datacenter == null)
			return;
		for(Vm v: this.vmMapId2Vm.values()) {
			SDNVm vm = (SDNVm)v;
			double cpuUtil = vm.getMonitoringValuesVmCPUUtilization()
				.getAverageValue(logTime - Configuration.monitoringTimeInterval, logTime);
			double bwRate = vm.getMonitoringValuesVmBwUtilization()
				.getAverageValue(logTime - Configuration.monitoringTimeInterval, logTime);
			org.cloudbus.cloudsim.sdn.memory.MemoryMonitor memMon =
				org.cloudbus.cloudsim.sdn.memory.MemoryTracker.getMonitor(vm.getId());
			double memPercent = 0;
			long memTotal = vm.getRam();
			if(memMon != null) {
				org.cloudbus.cloudsim.sdn.memory.MemoryData md = memMon.getLatest();
				if(md != null) {
					memPercent = md.getUsedPercent();
					memTotal = md.getTotalMemBytes();
				}
			}
			String vmName = vm.getName();
			int vmId = vm.getId();
			int operStatus = 0;
			int zeroOctetCount = 0;
			faultService.record(vmId, vmName, logTime, cpuUtil, memPercent, memTotal, bwRate, operStatus, zeroOctetCount);
		}
	}
