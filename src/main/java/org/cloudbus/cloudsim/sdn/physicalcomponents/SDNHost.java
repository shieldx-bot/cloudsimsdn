...
import org.cloudbus.cloudsim.sdn.memory.MemoryTracker;
import org.cloudbus.cloudsim.sdn.memory.MemorySnapshotHelpers;
...
//			System.err.println(CloudSim.clock()+":"+vm + " is reallocated: "+ mipsAllocatedAfter);
	...
	public void updateMonitor(double logTime, double timeUnit) {
		...
		// Update memory monitoring for hosted VMs
		updateMemoryMonitor();
	...
	private void updateMemoryMonitor() {
		for(Vm vm: getVmList()) {
			SDNVm tvm = (SDNVm)vm;
			double usedPercent = MemorySnapshotHelpers.getUsedRamPercentage(tvm, this);
			long totalMemBytes = tvm.getRam();
			tvm.recordMemorySnapshot(CloudSim.clock(), usedPercent, totalMemBytes);
		}
	}	
...
//		System.err.println(this.toString() +","+ processedMIs);
	...
	/**
	 * Collects metrics for a single VNF (VM) for online fault prediction.
	 *
	 * @param vm      the SDNVm representing the VNF
	 * @param logTime the current simulation time
	 * @return double[11] array with metrics in order:
	 *         [0] operStatus (0=up, 1=down)
	 *         [1] inOctDelta
	 *         [2] outOctDelta
	 *         [3] c0_usr (core 0 user %)
	 *         [4] c0_sys (core 0 system %)
	 *         [5] c0_idle (core 0 idle %)
	 *         [6] c1_usr (core 1 user %)
	 *         [7] c1_sys (core 1 system %)
	 *         [8] c1_idle (core 1 idle %)
	 *         [9] mem_status (0=Healthy, 1=Critical if usedPercent >= 0.9)
	 *         [10] used_percent (memory used percentage)
	 */
	public double[] collectVnfMetrics(SDNVm vm, double logTime) {
		double[] metrics = new double[11];

		metrics[0] = vm.getOperStatus() == 0 ? 0.0 : 1.0;
		metrics[1] = vm.getOctetsInDelta();
		metrics[2] = vm.getOctetsOutDelta();
		metrics[3] = vm.getCoreUserPercentage(0);
		metrics[4] = vm.getCoreSystemPercentage(0);
		metrics[5] = vm.getCoreIdlePercentage(0);
		metrics[6] = vm.getCoreUserPercentage(1);
		metrics[7] = vm.getCoreSystemPercentage(1);
		metrics[8] = vm.getCoreIdlePercentage(1);

		double usedPercent = MemorySnapshotHelpers.getUsedRamPercentage(vm, this);
		metrics[10] = usedPercent;
		metrics[9] = (usedPercent >= 0.9) ? 1.0 : 0.0;

		return metrics;
	}

	/**
	 * Collects metrics for all target VNFs (udm, amf, ausf) on this host.
	 *
	 * @param logTime the current simulation time
	 * @return double[33] array with concatenated metrics for udm, amf, ausf (11 each).
	 *         Missing VNFs are filled with 0.0.
	 */
	public double[] collectAllVnfMetrics(double logTime) {
		double[] allMetrics = new double[33];
		String[] vnfNames = {"udm", "amf", "ausf"};

		for (int i = 0; i < vnfNames.length; i++) {
			String targetName = vnfNames[i];
			SDNVm foundVm = null;

			for (Vm vm : getVmList()) {
				SDNVm sdVm = (SDNVm) vm;
				if (sdVm.getName().equalsIgnoreCase(targetName)) {
					foundVm = sdVm;
					break;
				}
			}
			...
			double[] vmMetrics;
			if (foundVm != null) {
				vmMetrics = collectVnfMetrics(foundVm, logTime);
			} else {
				vmMetrics = new double[11];
			}

			System.arraycopy(vmMetrics, 0, allMetrics, i * 11, 11);
		}
