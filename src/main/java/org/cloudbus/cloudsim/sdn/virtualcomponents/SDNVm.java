...
//			utilization = (double)monitoringProcessedBytesPerUnit / capacity;
	...
	public void recordMemorySnapshot(double timestamp, double usedPercent, long totalMemBytes) {
		MemoryTracker.record(this.getId(), timestamp, usedPercent, totalMemBytes);
	}
	...
	private long lastInOctets = 0;
	private long lastOutOctets = 0;
	private int operStatus = 0;

	public double getCoreUserPercentage(int peIndex) {
		if (peIndex < 0 || peIndex >= getNumberOfPes()) {
			return 0.0;
		}
		CloudletScheduler scheduler = getCloudletScheduler();
		if (scheduler instanceof CloudletSchedulerTimeSharedMonitor) {
			return ((CloudletSchedulerTimeSharedMonitor) scheduler).getPerPeUserPercentage(peIndex);
		}
		if (scheduler instanceof CloudletSchedulerMonitor) {
			try {
				java.lang.reflect.Method m = scheduler.getClass().getMethod("getPerPeUserPercentage", int.class);
				return (Double) m.invoke(scheduler, peIndex);
			} catch (Exception e) {
			}
		}
		return getAggregateUserPercentage() / getNumberOfPes();
	}

	private double getAggregateUserPercentage() {
		double util = 0.0;
		try {
			util = getMonitoringValuesVmCPUUtilization().getAverageValue(
					Math.max(0, CloudSim.clock() - Configuration.monitoringTimeInterval),
					CloudSim.clock());
		} catch (Exception e) {
		}
		return util;
	...
	public double getCoreSystemPercentage(int peIndex) {
		if (peIndex < 0 || peIndex >= getNumberOfPes()) {
			return 0.0;
		}
		CloudletScheduler scheduler = getCloudletScheduler();
		if (scheduler instanceof CloudletSchedulerTimeSharedMonitor) {
			return ((CloudletSchedulerTimeSharedMonitor) scheduler).getPerPeSystemPercentage(peIndex);
		}
		if (scheduler instanceof CloudletSchedulerMonitor) {
			try {
				java.lang.reflect.Method m = scheduler.getClass().getMethod("getPerPeSystemPercentage", int.class);
				return (Double) m.invoke(scheduler, peIndex);
			} catch (Exception e) {
			}
		}
	...
	public double getCoreIdlePercentage(int peIndex) {
		if (peIndex < 0 || peIndex >= getNumberOfPes()) {
			return 0.0;
		}
		CloudletScheduler scheduler = getCloudletScheduler();
		if (scheduler instanceof CloudletSchedulerTimeSharedMonitor) {
			return ((CloudletSchedulerTimeSharedMonitor) scheduler).getPerPeIdlePercentage(peIndex);
		}
		if (scheduler instanceof CloudletSchedulerMonitor) {
			try {
				java.lang.reflect.Method m = scheduler.getClass().getMethod("getPerPeIdlePercentage", int.class);
				return (Double) m.invoke(scheduler, peIndex);
			} catch (Exception e) {
			}
		}
		double userPct = getCoreUserPercentage(peIndex);
		return Math.max(0.0, 1.0 - userPct);
	}

	public long getOctetsInDelta() {
		long current = monitoringProcessedBytesPerUnit;
		long delta = current - lastInOctets;
		lastInOctets = current;
		return Math.max(0, delta);
	}

	public long getOctetsOutDelta() {
		long current = monitoringProcessedBytesPerUnit;
		long delta = current - lastOutOctets;
		lastOutOctets = current;
		return Math.max(0, delta);
	}
